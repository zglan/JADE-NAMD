...
#           print n_line
#           print len(line_each)
           self.current_state = np.zeros ( (n_line-2), dtype=np.int32 ).reshape(n_line-2,1)
           for i_line in range ( n_line-2 ) :
#                print  i_line+2, line_each[i_line+2], line_each[i_line+2].split()[2]
                self.current_state[i_line, 0] = int ( line_each[i_line+2].split()[2] )
