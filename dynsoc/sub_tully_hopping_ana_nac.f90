...
!         set the denmension of h_theta and rho_tmp_theta
           ...
           allocate (nac_x_eff(n_state, n_state, n_atom))
           allocate (nac_y_eff(n_state, n_state, n_atom))
           allocate (nac_z_eff(n_state, n_state, n_atom))
...
!
             do i=1,n_state
                 if (index_state .ne. i) then
                    ...
                    do k =1, n_atom
                        h_vv(i, index_state) = h_vv(i, index_state)  &
                                   + old_nac_x (i,index_state,k) * old_vel_x(k) &
                                   + old_nac_y (i,index_state,k) * old_vel_y(k) &
                                   + old_nac_z (i,index_state,k) * old_vel_z(k)
                    enddo
                    pro_hop(i) = - ( 2 * dtime *             &
                     ...
                     do k =1, n_atom
                        h_vv(i, index_state) = h_vv(i, index_state)  &
                                   + nac_x (i,index_state,k) * vel_x(k) &
                                   + nac_y (i,index_state,k) * vel_y(k) &
                                   + nac_z (i,index_state,k) * vel_z(k)
                     enddo
                     pro_hop(i) =  pro_hop(i) &
                                   -( 2 * dtime *             &
                                      real(conjg(rho(i,index_state)))  &
...
9999   format(i10, 1x, f15.8, 3x, a, 1x, i3, 1x, i3, 1x, f10.7, a, f10.7)
           ...
           deallocate (nac_x_eff)
           deallocate (nac_y_eff)
           deallocate (nac_z_eff)
           deallocate (rho_tmp)
           deallocate (work)
           deallocate (ematrix)
           deallocate (tmp1)
