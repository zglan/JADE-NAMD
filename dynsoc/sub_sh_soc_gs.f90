! surface hopping for ground state, spin cross over.
! 
! @ 2015-07-26
! @ dulikai @ qibebt
! n_state = 1 0 1 0 1
! i_state = 3
! the order of i_state is defined in the order of singlet, triplet
! 
! hop_e check hops below hop_e
!

      subroutine   sub_sh_soc_gs ( n_atom, &
                                 i_state, &
                                 n_state, md_state, &
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      character*2, dimension(n_atom)  ::  atom_label
       ...
       integer :: file_save_energy, file_save_traj, file_save_grad, & 
       ...
       double precision, dimension(n_state, n_state) :: soc_time, &
                                                     old_soc_time
                                                     
       double precision, dimension(n_state, n_state) :: couple_time, &
                                                     old_couple_time
...
! 9998  format (a500)
        ...
        call sub_save_state(n_atom, n_state, md_state, &
                                it, time, &
                                index_state, &
                                pes_all, &
                                gradient_x, &
        ...
        call sub_save_hop_all (n_atom, n_state, md_state, &
                              it,time, &
                              index_state, &
                              pes_all, &
                              nac_time, &
...
!      Begin the propagation      
       do it=1, ntime
           ...
           old_soc_time = soc_time
           old_couple_time = couple_time
...
!         write (*,*) "RANDOM", seed_random
          ...
          call sub_tully_hopping_num_soc (n_atom, &
                                    mass, &
                                    old_vel_x, &
                                    old_vel_y, &
                                    old_vel_z, &
