...
e_min = float(input('Minimum energy? '))

e_max = float(input('Maximum energy? '))

ne = int(input('How many bins? '))

e_sigma = float(input('HHFW? '))
...
for i_e in range(ne): 
    emission_energy[i_e]=e_min+float(i_e)*de
    filein4.write(''+str(emission_energy[i_e])+'    '+str(emission_intensity[i_e]/(n_freq-3))+' \n ')       
