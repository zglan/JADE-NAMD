...
class gau_log_parser():
    ...
    def __init__(self, config = {}):
        ...
        self.ci = {}        
        self.files = {'ao': "ch2nh2.log", 'mo': 'a.log'}
        ...
        if config != {}:
            ...
            self.files = {}
            self.files["ao"] = self.directory['overlap'] + "/" + files['gau_input'] 
            self.files["mo"] = self.directory['work'] + "/" + files['gau_input'] 
...
#   Check the DFT and TDDFT output    
    ...
    def get_basis(self):
        ...
        logfile = self.files['mo']
        fp = open(logfile, "r")
        pat = re.compile("AO basis set \(Overlap normalization\)")
        line = "XXX"
        while line != "":
            line = fp.readline()
            m1 = pat.search(line)
        ...
        pat_end = re.compile("symmetry adapted basis functions")
        pat = re.compile("\s+Atom\s+(\S+)")
        line_all = []
        while line != "":
            line = fp.readline()
            m = pat.search(line)
            m_end = pat_end.search(line)
            if m is not None:
        ...
        line_unique = []
        line_name = "X"
        for i in xrange(len(line_all)):
            line = line_all[i]
            if line_name != line[0:10]:
                line_name = line[0:10]
                line_unique.append(line_all[i])
                 
        index = []
        atom = []
        for line in line_unique:
            record = line.split()
            atom_name = re.sub("\d", "", record[1])
            atom.append(atom_name)
            index.append(int(record[7]))
        record = line_all[-1].split()
        index.append(int(record[9]))

        basis = []
        for i in xrange(len(index)-1):
            n_bf = index[i+1] - index[i]
            basis.append(n_bf)
...
# --------------------------------------------------------------------------------------------------
	...
	xx = record
        ...
        for i in range(0, n_basis): # i_end eq n_double_basis
            ...
            record = line[20:].split()
	    n_record = len(record)
	    if n_record != n_col_tmp:
                record = []
	        for ik in xrange(n_col_tmp):
            #        print i, 21+ik*10, line[21+ik*10:21+ik*10+10]
		    #print i, i_block, n_col_tmp, n_record, line[20:].split()
                    record.append(float(line[21+ik*10:21+(ik+1)*10]))
		    #print len(record)
            for j in range(i_start, i_start + n_col_tmp):  
                j_col = j -i_start  #                  
...
# --------------------------------------------------------------------------------------------------
    ...
    def __read_ci_block(self, file_in, i_state):
        ...
        grp = m.group()
        record = grp.split()
        self.ci['state'][i_state]['energy'] = float(record[0]) / 27.112
        ...
        while 1 :
            ...
            # print i_ci_1, i_ci_2, value, flag
        ...
        for i_state in range(1, n_state):
            ...
            for i_ci_1 in range(n_occ):
                for i_ci_2 in range(n_vir):
                   one_state[i_ci_1][i_ci_2]  =  one_state1[i_ci_1][i_ci_2] + one_state2[i_ci_1][i_ci_2]
