subroutine sub_reshape_n_atom_to_n_mode(x_nuc_cur, &
                  p_nuc_cur, &
                  grad, &
                  dij_cur, &
...
do i_atom = 1, n_atom
  p_nuc_cur(3 * (i_atom - 1) + 1) = vel_x(i_atom)
  p_nuc_cur(3 * (i_atom - 1) + 2) = vel_y(i_atom)
  p_nuc_cur(3 * (i_atom - 1) + 3) = vel_z(i_atom)
enddo

do i_atom = 1, n_atom
  grad(:,3 * (i_atom - 1) + 1) = gra_all_x(:,i_atom)
  grad(:,3 * (i_atom - 1) + 2) = gra_all_y(:,i_atom)
  grad(:,3 * (i_atom - 1) + 3) = gra_all_z(:,i_atom)
enddo
