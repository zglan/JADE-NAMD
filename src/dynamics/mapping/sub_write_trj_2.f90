subroutine sub_write_trj_2(i_step, &
                         x_cur, p_cur, &
                         x_elec, p_elec, &
                         Ham)
...
!write(*,*) mat_tmp
do i = 1, n_state
  mat_pop_dia(i,i) = 0.5 * ( x_elec(i)**2 + p_elec(i)**2 - ggamma)
enddo

do i = 1, n_state-1
  do j = i+1, n_state
    mat_pop_dia(i,j) = 0.5 * ( x_elec(i) * x_elec(j) + p_elec(i) * p_elec(j) )
    mat_pop_dia(j,i) = mat_pop_dia(i,j)
  enddo
enddo
...
if (label_debug >= 2 ) then
  ...
  write(f_trj_elec_debug, 9999) i_step, x_elec(:), p_elec(:)
...
do i = 1, n_state
  write(f_trj_elec_restart, 9998) x_elec(i), p_elec(i)
