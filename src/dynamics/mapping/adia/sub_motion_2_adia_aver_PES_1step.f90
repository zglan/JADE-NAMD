subroutine sub_motion_2_adia_aver_PES_1step(x_elec, &
                            p_elec, &
                            x_nuc_cur, &
                            p_nuc_cur, &
                            ene_adia_cur, &
...
integer i_step
real(kind=8) dHmn_div_dX_adia_pre(n_mode, n_state, n_state)
real(kind=8) dHmn_div_dX_adia_cur(n_mode, n_state, n_state)
...
if(label_debug >= 2) write(*,*) "cur"
call sub_p_to_p_kin(p_nuc_cur, p_nuc_kin_cur, &
                    dij_cur, &
                    x_elec, p_elec)
if(label_debug >= 2) write(*,*)

if(label_debug >= 2) write(*,*) "pre"                
