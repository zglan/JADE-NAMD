subroutine sub_write_trj_2_adia_traj_adjusted(i_step, &
                         x_elec, p_elec, &
                         x_nuc, p_nuc, &
                         mat_U, gammas)
use mod_main
...
do i = 1, n_state
  mat_pop_adia(i,i) = 0.5 * ( x_elec(i)**2 + p_elec(i)**2 - gammas(i) )
enddo
...
mat_pop_dia = matmul(mat_U, mat_pop_adia)
mat_pop_dia = matmul(mat_pop_dia, transpose(mat_U))
...
if(label_debug >= 2) then
  write(f_trj, 9999) i_step, x_nuc, p_nuc
!  do i = 1, n_mode
