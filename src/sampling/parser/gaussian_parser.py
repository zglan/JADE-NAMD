...
class gaussian_parser():
    ...
    def __read_gaussian_log_geom(self):
        ...
        for i in xrange(n_atom):
            ...
            atom['atom_label'] = emt.number2label(atom_number)
            geom[i] = atom
    ...
    def __read_gaussian_log_freq_block2(self, fp, i_block):
        """
        read in one block and assign variables: for the case of no hpmode, low accuracy
        [x.xx]
        """
        n_atom = self.dim['n_atom']
        n_col = 3
    ...
    def __read_gaussian_log_freq2(self):
        """
        read in every normal mode
        for the case without hpmode x.xx
        """
        filename = self.files['log']
        n_atom = self.dim['n_atom']
        fp = open(filename, "r")
        ...
        for i in xrange(n_block):
            self.__read_gaussian_log_freq_block2(fp, i)
    ...
    def read_log(self):
        ...
        """
        line = raw_input("GAUSSIAN LOG FREQ TYPE: <hpmode/other>: [default: hpmode]\n > ")
        freq_type = line.strip()
        filename = self.files['log']
        file_out = open(filename, "r")
        # read gaussian geometry
        self.__read_gaussian_log_dim()
        self.__read_gaussian_log_geom()
        if freq_type == "hpmode" or freq_type == "":
            self.__read_gaussian_log_freq()
        else:
            self.__read_gaussian_log_freq2()
