...
!      ---- local variables ---
       ...
       double precision, allocatable, dimension(:) :: random_1, &
                                                      random_2, &
                                                      random_3
       double precision, allocatable, dimension(:) :: p_momentum
       ...
       i_random = 1
       do i_geom = 1, n_geom
          write (*,*)   "Geom", i_geom, "Reactive coordinates"
          do  j_mode = 1, number_negative
              
            
              if (i_geom  .lt. n_geom/2 ) then 
                p_momentum (i_random) = ( -2 * kB * t_k * &
                                          dlog(1 - random_1(i_random) ) &
                                         ) **0.5
              else 
                p_momentum (i_random) =-( -2 * kB * t_k * &
                                          dlog(1 - random_1(i_random) ) &
...
!              write (*,*)  kB, t_k, dlog(1 - random_1(i_random) )
!              write (*,*)  random_1(i_random), p_momentum (i_random), ( abs( frequency(j_mode) / &
!                                               TOCM                &
!                                             )**0.5                &
!                                        )


              p_momentum (i_random)  =  p_momentum (i_random) /   &
                                        ( abs( frequency(j_mode) / &
                                               TOCM                &
                                             )**0.5                &
                                        )
              
              write (*,*) "for Mode", j_mode       
              
              write (*,*) "Momentum", p_momentum (i_random)
              i_random = i_random + 1 
          enddo
       enddo
       ...
       pro = 0.d0
       do j_mode = number_negative+1, n_mode
...
!            write (*,*) j_mode, i_level, pro(j_mode, i_level)
!            write (*,*) i_level, frequency(j_mode)
!            write (*,*)  exp ( - ( (i_level-1) * frequency(j_mode) ) &
!                                  / ( KB*t_k*TOCM )  &
!                                )
!            write (*,*) ( 1- exp (   - frequency(j_mode)      &
!                                       / (KB*t_k*TOCM)        &
!                                 )                            &
!                        )
!            write (*,*) exp ( - ( (i_level-1) * frequency(j_mode) ) &
...
992          write (*,*) "Mode and level", &
       ...
       do i_geom = 1, n_geom 
          ...
          do  j_mode = 1, n_mode
              ...
              if ( j_mode .le. number_negative) then
                 random_gau1(i_total)  = 0
                 random_gau2(i_total)  = p_momentum (i_random)
              endif
              if ( j_mode .ge. number_negative+1) then
