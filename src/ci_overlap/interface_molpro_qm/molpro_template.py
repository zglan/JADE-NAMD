...
class molpro_template():
    ...
    def __init__(self, config={}):
        ...
        self.template = {}
        self.template_cmp = {'routine': {}, 'title': '', 'mol': {}, 'tail': '{}'}
        # i/o pointer                      
        self.pointer = {"content": [], "i_line": 0}
        self.files = {'molpro': './MOLPRO_EXAM/molpro_template.inp', 'template': 'template.json'}

        if config != {}:
            root_dir = config['root']
            dirs = config['dirs']
            files = config['files']
            molpro_file = root_dir + "/" + dirs['template'] + "/" + files['molpro_template']
