...
class gau_log_wf_parser():
    """
    parse gau. log file for w.f. info. that is mo & ci coeff.
    for a pure QM calculation,
    the back-ground point charge is supported, if possible
    suppose only one TD layer,..
    """
    def __init__(self, config = {}):
        """ wavefunction information """
        self.config = config
        # dimension of gaussian output. column
        self.dim = {'n_col': 5, 'layer': 'high-model'}
        self.mo = {"coeffs": [], "energy": [], "alpha": [], "beta": [], \
                "alpha_energy": [], "beta_energy": [], 'spin': 0} 
                # 0 for close shell case, 1 for open shell case
        self.ci = {}        
        self.files = {'mo': "high-model.log"}
        self.directory = {'root': './' }
        if config != {}:
...
#   Check the DFT and TDDFT output    
    ...
    def get_gradient(self):
        ...
        logfile = self.files['high-model-log']
        file_in = open(logfile, "r")
        # locate data.
        ...
        while line != "":
            ...
            if m is not None:    
                ...
                break
        # dump the data
        file_out = open(filename + "-qm_gradient.dat", "w")
        while line != "": 
            line = file_in.readline()           
    ...
    def __distrib_ci_matrix(self):
        ...
        for i_state in range(1, n_state):
            ...
            if ci_type == "X+Y":
                ...
                print "no other option for log keyword in gaussian"
                exit(1)
...
#                       print "ci_all", i_all-1, ci_all[i_all-1] 
    ...
    def __wrt_ci_td_energy(self):
        ...
        state = self.ci['state']
        n_state = self.ci['n_state']             
        fileout1=open('high-model-energy.dat', 'w')
