...
class gau_rwf_wf_parser():
    ...
    def __init__(self, config = {}):
        ...
        """
        self.config = config
        self.files = {'chk': "high-model.chk", 'mo': 'high-model.log'}
        self.dim = {'layer': 'high-model'}  
        self.ci = {}
        self.mo = {}     
        ...
        if config != {}:
            ...
            self.files["chk"] = self.directory['work'] + "/" + files['high-model-chk'] 
        ...
        if 'is_do_ci_assignment' in config.keys():
            self.is_do_ci_assignment = config['is_do_ci_assignment']
        if 'ci_assignment_factor' in config.keys():
        ...
        for i_state in range(1, n_state):                    
            ...
            for i in range(noccA):
                for j in range(nvirA):
                    ...
                    # F = (A-B)^(-1/2) |X+Y>
                    # so c_ia = F is an option, J. Chem. Phys. 135, 054105, (2011)
                    # follow casida's assignment, 
                    # JMS: THEOCHEM 2009, 914, 3
                    # sqrt((A-B)/omega) F
