...
#  ---------------------------------------------------------------------
#  Generate the RI-CC2 or ADC(2) input for Turbomole
#-----------------------------------------------------------------------------------------   
...
def modify_control_ricc (n_atom, n_state, index_state) :
...
#   Set the number of states involved in the CC2 or ADC2 work.
         
        i_find_label = re.search('\$excitations', line_each[i_line])
        if  i_find_label is not None:
            i_line = i_line+1 
            estate_line  = 'irrep=a  multiplicity=  1  nexc=  '
            estate_line += str(n_state-1)
            estate_line += '  npre=  '
            estate_line += str(n_state-1)
            estate_line += '  nstart=  '
            estate_line += str(n_state-1)   
            line_each[i_line] = estate_line

#   Set which state to compute the gradient
        if index_state > 1:
            i_find_label = re.search('geoopt', line_each[i_line])
            if  i_find_label is not None:
                exopt_line  = 'geoopt   '
                exopt_line += str(line_each[i_line-1])
                exopt_line += '    state=(a '
                exopt_line += str(int(index_state)-1)
                exopt_line += ')'
	        line_each[i_line] = exopt_line
